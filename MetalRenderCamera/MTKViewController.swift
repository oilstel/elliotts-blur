...
open class MTKViewController: UIViewController {
    ...
    fileprivate func initializeMetalView() {
        ...
        metalView.transform = metalView.transform.scaledBy(x: 1.18, y: 1.18) // Elliott (scale values)
    ...
    internal var blur : MPSImageGaussianBlur?
    ...
    fileprivate func initializeRenderPipelineState() {
        ...
        blur = MPSImageGaussianBlur(device: device, sigma: 30) // Elliott (blur amount)
...
extension MTKViewController: MTKViewDelegate {
    ...
    public func draw(in: MTKView) {
        ...
        autoreleasepool {
            guard
             ...
             //if (textureBlurred == false) {
                //blur?.encode(commandBuffer: commandBuffer, inPlaceTexture: &texture, fallbackCopyAllocator: nil)
                //textureBlurred = true
            //}
    ...
    private func render(texture: MTLTexture, withCommandBuffer commandBuffer: MTLCommandBuffer, device: MTLDevice) {
        ...
        var front:float_t = 1.0

        if (cameraFront) {
            front = -1.0
        }
        
        encoder.setVertexBytes(&front, length: MemoryLayout.size(ofValue:front), index: 0)
        ...
        var p = currentDrawable.texture
        blur?.encode(commandBuffer: commandBuffer, inPlaceTexture: &p, fallbackCopyAllocator: nil)
